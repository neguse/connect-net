...
internal static class ConnectClientStreamHandler
{
        ...
        ICodec codec)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                var responseBytes = codec.SerializeToArray(result);
                if (responseCompressor != null)
                {
                    responseBytes = responseCompressor.CompressToArray(responseBytes);
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        while (true)
        {
            // Per-envelope idle deadline: a slow-loris client that opens the stream and then
            // dribbles bytes (or stalls between envelopes) cannot pin this server task forever.
